...
app.post("/api/notes", upload.single("file"), (req, res) => {
  ...
  if (!title || !course || !section || !req.file) return res.status(400).json({ error: "All fields required" });
...
app.get("/api/notes/:id/download", (req, res) => {
  ...
  if (!note || !note.file) return res.sendStatus(404);
  res.download(path.join(__dirname, "uploads", note.file));
});

app.use(express.static(path.join(__dirname, "public"))); // optional if html/js in public
